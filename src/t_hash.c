 ...
 * If values is NULL, creates empty array. Otherwise copies values. */
...
robj *createTmplHashObject(hashTemplate *tmpl, sds *values) {
    ...
    if (hashFitsInTmplLp(tmpl->field_count, values)) {
        ...
        o = createObject(OBJ_HASH, hashTemplateArrayCreate(tmpl, values));
...
int hashTypeNext(hashTypeIterator *hi, int skipExpiredFields) {
    ...
    if (hi->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        unsigned long long field_count = lpLength(lp) - 1;

        /* Advance to next field. */
        hi->tmpl_index++;
        if ((unsigned long long)hi->tmpl_index >= field_count) return C_ERR;

        /* Find value pointer in listpack (skip template ID). */
        if (hi->tmpl_index == 0) {
            hi->vptr = hashTemplateLpFirstValue(lp);
        } else {
            hi->vptr = lpNext(lp, hi->vptr);
        }
        if (!hi->vptr) return C_ERR;
    } else if (hi->encoding == OBJ_ENCODING_TMPL_ARRAY) {
        hashTemplateArray *hta = hi->subject->ptr;
        unsigned long long field_count = hta->tmpl->field_count;
                           ...
                           uint64_t *expireTime)
{
        ...
        hi->encoding == OBJ_ENCODING_LISTPACK_EX)
    {
        ...
        *vlen = eleLen;
        ...
        hashTemplate *tmpl = hashTemplateLpGetTemplate(hi->subject->ptr);
        if (what & OBJ_HASH_KEY) {
            /* Return field name from tmpl. */
            sds field = tmpl->fields[hi->tmpl_index];
        ...
        if (what & OBJ_HASH_KEY) {
            /* Return field name from tmpl. */
            sds field = hta->tmpl->fields[hi->tmpl_index];
            *vstr = (unsigned char*) field;
            *vlen = sdslen(field);
        } else {
...
static void hashTmplLpConvertToArray(robj *o) {
    ...
    hashTemplateArray *hta = hashTemplateArrayCreate(tmpl, values);
    zfree(values);
    hashTemplateLpFree(lp);
...
static void himportPrepareCommand(client *c) {
    if (c->argc < 4) {
        addReplyErrorArity(c);
        return;
    }
...
static void himportSetCommand(client *c) {
    ...
    /* Build values array and propargv.
    ...
    /* Prepare propargv for HSETC propagation. */
    robj *stack_propargv[2 + HASH_TMPL_STACK_ENTRIES * 2];
    robj **propargv = (field_count <= HASH_TMPL_STACK_ENTRIES) ?
                      stack_propargv :
    ...
    for (unsigned long long i = 0; i < field_count; i++) {
        ...
        /* Build propargv entry. */
        propargv[2 + i * 2] = tmpl->field_objs[i];
    ...
    if (propargv != stack_propargv) zfree(propargv);
...
static void himportDiscardCommand(client *c, int all) {
    /* HIMPORT DISCARD <template> */
    /* HIMPORT DISCARDALL */
    if ((all && c->argc != 2) || (!all && c->argc != 3)) {
        addReplyErrorArity(c);
        return;
    }
    ...
    /* Both DISCARD and DISCARDALL always return OK. */
    addReply(c, shared.ok);
}

void himportCommand(client *c) {
    if (c->argc < 2) {
        addReplyErrorArity(c);
        return;
    }
    ...
    if (!strcasecmp(sub->ptr, "set")) {
        himportSetCommand(c);
    } else if (!strcasecmp(sub->ptr, "prepare")) {
        himportPrepareCommand(c);
    } else if (!strcasecmp(sub->ptr, "discard")) {
        himportDiscardCommand(c, 0);
    } else if (!strcasecmp(sub->ptr, "discardall")) {
        himportDiscardCommand(c, 1);
    } else {
        addReplyError(c,"ERR unknown HIMPORT subcommand");
    }
}

/* HSETC key field1 value1 [field2 value2 ...]
 * Creates a tmpl-based hash. If key exists, it is deleted first.
 * This command is propagated by HIMPORT SET for replication/AOF. */
typedef struct { sds field; sds value; } hsetcPair;

static int hsetcPairCmp(const void *a, const void *b) {
    return sdscmplen(((hsetcPair *)a)->field, ((hsetcPair *)b)->field);
}

/* Per-client cache for HSETC field order - avoids re-sorting. */
typedef struct hsetcCache {
    unsigned long long field_count;
    sds *fields;              /* Cached field names (in user order) */
    int *value_order;         /* Maps user order -> template order */
    hashTemplate *tmpl;
} hsetcCache;
...
void hsetcCacheFree(client *c) {
    ...
    zfree(cache->fields);
    zfree(cache->value_order);
    if (cache->tmpl) hashTemplateReleaseFromClient(cache->tmpl);
    zfree(cache);
    c->hsetc_cache = NULL;
}

/* Check if incoming fields match cached order exactly. */
static int hsetcCacheMatch(hsetcCache *cache, client *c, unsigned long long field_count) {
    ...
    for (unsigned long long i = 0; i < field_count; i++) {
        sds f = c->argv[2 + i * 2]->ptr;
        if (sdscmplen(cache->fields[i], f) != 0) return 0;
    }
    return 1;
}
...
void hsetcCommand(client *c) {
    ...
    /* Fast path: fields match cached order - skip sort. */
    if (hsetcCacheMatch(cache, c, field_count)) {
        tmpl = cache->tmpl;
        values = (field_count <= HASH_TMPL_STACK_ENTRIES) ?
                 stack_values : zmalloc(sizeof(sds) * field_count);
        for (unsigned long long i = 0; i < field_count; i++) {
            values[cache->value_order[i]] = c->argv[3 + i * 2]->ptr;
        }
    } else {
        /* Slow path: sort and update cache. */
        hsetcPair stack_pairs[HASH_TMPL_STACK_ENTRIES];
        hsetcPair *pairs = (field_count <= HASH_TMPL_STACK_ENTRIES) ?
                           stack_pairs : zmalloc(sizeof(hsetcPair) * field_count);

        for (unsigned long long i = 0; i < field_count; i++) {
            pairs[i].field = c->argv[2 + i * 2]->ptr;
            pairs[i].value = c->argv[3 + i * 2]->ptr;
        }
        qsort(pairs, field_count, sizeof(hsetcPair), hsetcPairCmp);
        ...
        if (pairs != stack_pairs) zfree(pairs);
        ...
        cache->value_order = zmalloc(sizeof(int) * field_count);
        cache->tmpl = tmpl;
        tmpl->client_refcount++;  /* Hold reference for cache. */

        for (unsigned long long i = 0; i < field_count; i++) {
            cache->fields[i] = sdsdup(c->argv[2 + i * 2]->ptr);
            /* Find position in sorted template. */
            sds f = cache->fields[i];
            for (unsigned long long j = 0; j < field_count; j++) {
                if (sdscmplen(f, tmpl->fields[j]) == 0) {
                    cache->value_order[i] = j;
                    break;
                }
            }
        }
...
void hrandfieldWithCountCommand(client *c, long l, int withvalues) {
    ...
    * elements inside the hash: simply return the whole hash. */
    ...
    /* CASE 2.5b Template-based hashes. Iterate and return unique elements. */
    if (hash->encoding == OBJ_ENCODING_TMPL_LP ||
        hash->encoding == OBJ_ENCODING_TMPL_ARRAY)
    {
        hashTypeIterator hi;
        hashTypeInitIterator(&hi, hash);
        while (hashTypeNext(&hi, 0) != C_ERR && count--) {
            if (withvalues && c->resp > 2)
                addReplyArrayLen(c, 2);
            addHashIteratorCursorToReply(c, &hi, OBJ_HASH_KEY);
            if (withvalues)
...
void hashTemplateDisassembleLowRefTemplates(size_t min_keys) {
    if (!htemplates->registry || min_keys == 0) return;

    /* Collect templates to disassemble (can't modify dict while iterating). */
    list *to_disassemble = listCreate();
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        unsigned long long key_ref;
        atomicGet(tmpl->key_refcount, key_ref);
        if (key_ref > 0 && key_ref < min_keys) {
            listAddNodeTail(to_disassemble, tmpl);
        }
    }
    dictReleaseIterator(di);

    if (listLength(to_disassemble) == 0) {
        listRelease(to_disassemble);
        return;
    }

    serverLog(LL_NOTICE, "Disassembling %lu templates with fewer than %zu keys",
              listLength(to_disassemble), min_keys);

    /* For each tmpl, scan all DBs and convert matching hashes. */
    listIter li;
    listNode *ln;
    listRewind(to_disassemble, &li);

    while ((ln = listNext(&li)) != NULL) {
        hashTemplate *tmpl = listNodeValue(ln);
        ...
        for (int dbid = 0; dbid < server.dbnum; dbid++) {
            ...
            while ((de = kvstoreIteratorNext(&kvs_it)) != NULL) {
                robj *o = dictGetKV(de);
                if (o->type != OBJ_HASH) continue;
                if (o->encoding != OBJ_ENCODING_TMPL_LP &&
                    o->encoding != OBJ_ENCODING_TMPL_ARRAY) continue;

                hashTemplate *obj_tmpl = NULL;
                if (o->encoding == OBJ_ENCODING_TMPL_LP) {
                    obj_tmpl = hashTemplateLpGetTemplate(o->ptr);
                } else {
                    obj_tmpl = ((hashTemplateArray *)o->ptr)->tmpl;
                }

                if (obj_tmpl != tmpl) continue;

                /* Convert to regular listpack. */
                unsigned char *new_lp = lpNew(0);
                int fc = tmpl->field_count;
                ...
                for (int i = 0; i < fc; i++) {
                    ...
                    if (o->encoding == OBJ_ENCODING_TMPL_LP) {
                        ...
                        unsigned int vlen;
                        long long vll;
                        unsigned char *vptr = hashTemplateLpGet(old_lp, i, &vlen, &vll);
                        if (vptr) {
                            new_lp = lpAppend(new_lp, vptr, vlen);
                        } else {
                            char buf[32];
                            int len = ll2string(buf, sizeof(buf), vll);
                            new_lp = lpAppend(new_lp, (unsigned char *)buf, len);
                        }
                    } else {
                        hashTemplateArray *hta = o->ptr;
                        sds val = hashTemplateArrayGet(hta, i);
                        if (val) {
                            new_lp = lpAppend(new_lp, (unsigned char *)val, sdslen(val));
                        } else {
                            new_lp = lpAppend(new_lp, (unsigned char *)"", 0);
                        }
                    }
                }

                /* Free old structure and update object. */
                if (o->encoding == OBJ_ENCODING_TMPL_LP) {
                    hashTemplateLpFree(o->ptr);
                } else {
                    hashTemplateArrayFree(o->ptr);
