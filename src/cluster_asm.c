...
void asmTrimJobProcessPending(void) {
    ...
    /* Check if we can start the trim job. We check replica pause to ensure we
     * can propagate TRIMSLOTS command to replicas. */
    static int logged = 0;
...
void asmActiveTrimCycle(void) {
    ...
    if (slot == -1) {
        ...
        /* Start the next trim job as soon as the current one completes. This
         * avoids gaps so modules are notified immediately and can keep
         * filtering out unowned keys. */
        if (listLength(asmManager->active_trim_jobs) != 0)
            asmActiveTrimStart();
    }

    long long elapsed = ustime() - start;
    latencyAddSampleIfNeeded("trim-cycle", elapsed / 1000);
}
